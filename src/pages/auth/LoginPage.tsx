...
import { Layers, ShieldCheck, User, ArrowRight, Lock, Mail, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const handleQuickLogin = async (quickEmail: string) => {
    ...
    setErrorMessage('');
    try {
      await switchUser(quickEmail);
      success('Logged in as demo persona', quickEmail);
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 className="text-2xl font-bold tracking-tight text-slate-900">DAYFLOW</h2>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
        {/* Quick Demo Login Preset Cards for Hackathon Judges */}
        <div className="mb-6 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">1-Click Hackathon Demo Logins</span>
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            <button
              id="quick-login-admin"
              type="button"
              onClick={() => handleQuickLogin('admin@dayflow.io')}
              className="flex flex-col items-start p-3 bg-slate-50 hover:bg-blue-50/70 border border-slate-200 hover:border-blue-300 rounded-xl transition-all text-left group"
            >
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900 group-hover:text-blue-600">
                <ShieldCheck className="w-4 h-4 text-blue-600" />
                <span>Sarah Jenkins</span>
              </div>
              <span className="text-[10px] text-slate-500 mt-0.5">Admin / HR Lead</span>
            </button>
            ...
            >
              ...
              <span className="text-[10px] text-slate-500 mt-0.5">Employee (Engineer)</span>
            </button>
          </div>
        </div>
        ...
        <div className="bg-white py-8 px-6 shadow-sm rounded-2xl border border-slate-200 sm:px-10">
          <form onSubmit={handleLogin} className="space-y-4 text-xs">
            ...
            <div className="pt-2">
              ...
              >
                <span>{isLoading ? 'Signing In...' : 'Sign In to Dayflow'}</span>
